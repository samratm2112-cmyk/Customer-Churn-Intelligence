...
def create_churn_vs_retention_chart(probabilities: np.ndarray) -> go.Figure:
    ...
    fig = go.Figure(data=[
        go.Bar(
            ...
            x=['Probability'],
            y=[retention_prob],
            marker=dict(color='#51cf66', line=dict(color='rgba(81, 207, 102, 0.5)', width=2)),
            text=f'{retention_prob:.1%}',
            textposition='auto',
        ),
        go.Bar(
            name='Churn',
            x=['Probability'],
            y=[churn_prob],
            marker=dict(color='#ff6b6b', line=dict(color='rgba(255, 107, 107, 0.5)', width=2)),
            text=f'{churn_prob:.1%}',
            textposition='auto',
        )
    ])
...
def create_risk_factor_analysis(user_input: Dict, churn_prob: float) -> go.Figure:
    ...
    # Tenure factor (0-1 scale, lower tenure = higher risk)
    ...
    fig = go.Figure(data=[go.Bar(
        x=factors,
        y=scores,
        marker=dict(color=colors, line=dict(color='rgba(102, 126, 234, 0.3)', width=1)),
        text=[f'{s:.0%}' for s in scores],
        textposition='auto',
        hovertemplate='%{x}<br>Risk Factor: %{y:.0%}<extra></extra>'
    )])
...
def create_what_if_analysis(user_input: Dict) -> go.Figure:
    """Create what-if scenario analysis."""
    scenarios = ['Current\nProfile', 'Add Tech\nSupport', 'Longer\nContract', 'Both\nActions']
    
    # Simulate churn probabilities for scenarios
    # Current scenario
    input_data = preprocess_input(user_input)
    _, probs = predict_churn(input_data)
    current_churn = probs[1]
    
    # Add tech support
    input_with_support = user_input.copy()
    input_with_support['tech_support'] = 'Yes'
    input_data2 = preprocess_input(input_with_support)
    _, probs2 = predict_churn(input_data2)
    support_churn = probs2[1]
    
    # Longer contract
    input_longer = user_input.copy()
    if user_input['contract'] == 'Month-to-month':
        input_longer['contract'] = 'Two year'
    input_data3 = preprocess_input(input_longer)
    _, probs3 = predict_churn(input_data3)
    contract_churn = probs3[1]
    
    # Both actions
    input_both = user_input.copy()
    input_both['tech_support'] = 'Yes'
    if user_input['contract'] == 'Month-to-month':
        input_both['contract'] = 'Two year'
    input_data4 = preprocess_input(input_both)
    _, probs4 = predict_churn(input_data4)
    both_churn = probs4[1]
    
    churn_probs = [current_churn, support_churn, contract_churn, both_churn]
    
    fig = go.Figure(data=[go.Bar(
        x=scenarios,
        y=churn_probs,
        marker=dict(
            color=['#ff6b6b', '#ffd43b', '#ffd43b', '#51cf66'],
            line=dict(color='rgba(102, 126, 234, 0.3)', width=2)
        ),
        text=[f'{p:.1%}' for p in churn_probs],
        textposition='auto',
        hovertemplate='%{x}<br>Churn Risk: %{y:.1%}<extra></extra>'
    )])
...
def display_results(prediction: int, probabilities: np.ndarray, insights: list):
    ...
    if churn_prob >= 0.65:
        ...
        risk_class = "risk-low"  # Using low for moderate to avoid harsh colors
